...
1. 断言
   ...
   | 名称         | 语法         | 说明                                          |
   | ------------ | ------------ | --------------------------------------------- |
   | 正向先行断言 | (?=pattern)  | 匹配pattern前面的内容，不包括pattern          |
   | 正向后行断言 | (?<=pattern) | 匹配pattern表达式的后面的内容，不包括其本身   |
   | 负向先行断言 | (?!pattern)  | 匹配非pattern表达式前面的内容，不返回其本身   |
   | 负向后行断言 | (?<!pattern) | 匹配非pattern表达式的后面的内容，不返回其本身 |

2. 捕获

   | 语法           | 说明                                                         |
   | -------------- | ------------------------------------------------------------ |
   | (exp)          | 数字捕获，根据数字顺序获取捕获组，0为整个匹配，1为第一个匹配 |
   | (?\<name\>exp) | 命名捕获，可根据名字获取捕获组                               |
